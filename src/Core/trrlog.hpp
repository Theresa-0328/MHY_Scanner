...
#define TRRLOG_BEGIN_NAMESPACE namespace trrlog{
#define TRRLOG_END_NAMESPACE }

#define DETAILS_BEGIN_NAMESPACE namespace details{
...
inline
LogLevel log_level_from_name(std::string level)
{
#define _FUNCTION(name) if (level== #name) return LogLevel::name;
	TRRLOG_FOREACH_LOG_LEVEL(_FUNCTION)
#undef _FUNCTION
